...
public class SelectableTreeFragment extends Fragment
{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
    {
        ...
        for (int i = 0;i < qServicesTypeSize;i++)
        {
            ...
            treeNodeList2.clear();
            if (qServiceTypeList.get(i).toString().equals("Financial Services"))
            {
                treeNodeList.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceTypeList.get(i).toString(),R.string.fa_credit_card)).setViewHolder(new ProfileHolder(getActivity())));
            }
            else if (qServiceTypeList.get(i).toString().equals("Government Services"))
            {
                treeNodeList.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceTypeList.get(i).toString(),R.string.fa_globe)).setViewHolder(new ProfileHolder(getActivity())));
            }
            //treeNodeList.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceTypeList.get(i).toString(),R.string.ic_star)).setViewHolder(new ProfileHolder(getActivity())));
            ...
            {
                if (qServiceProviderName.get(j).toString().equals("Mbabane Govt Hospital"))
                {
                    treeNodeList2.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceProviderName.get(j), R.string.fa_hospital_o)).setViewHolder(new SelectableHeaderHolder_2(getActivity())));
                }
                else if (qServiceProviderName.get(j).toString().equals("Swaziland Revenue Authority"))
                {
                    treeNodeList2.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceProviderName.get(j), R.string.fa_paperclip)).setViewHolder(new SelectableHeaderHolder_2(getActivity())));
                }
                else if (qServiceProviderName.get(j).toString().equals("FNB"))
                {
                    treeNodeList2.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceProviderName.get(j), R.string.fa_money)).setViewHolder(new SelectableHeaderHolder_2(getActivity())));
                }
                else if (qServiceProviderName.get(j).toString().equals("SwaziBank"))
                {
                    treeNodeList2.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceProviderName.get(j), R.string.fa_money)).setViewHolder(new SelectableHeaderHolder_2(getActivity())));
                }
                //treeNodeList2.add(new TreeNode(new IconTreeItemHolder.IconTreeItem(qServiceProviderName.get(j), R.string.ic_star)).setViewHolder(new SelectableHeaderHolder_2(getActivity())));
                treeNodeList.get(i).addChildren(treeNodeList2.get(j));
                String provName = qServiceProviderName.get(j);
                getServicePID = db.getQServicePid(provName);
                ...
                customfillFolder(treeNodeList2.get(j), serviceNameColl, provName);
                serviceNameColl.clear();
                //treeNodeList2.clear();
            }
